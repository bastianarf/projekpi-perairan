    ...
    <div class="row row-cols-10 shadow rounded-lg p-3 justify-content-center m-0" style="background-color: rgb(0, 183, 255)">
        ...
        <div class="col-9">
            ...
            <div class="tab-content bg-light rounded-lg shadow p-4" id="v-pills-tabContent">
                ...
                @endif
                <h3 class="text-center">Edit Tanggal Rincian<br></h3>
                <hr>
                ...
                @endif
                <form method="POST" action="{{ Route('Tanggalrincian') }}/{{ $sppd->id }}/{{$tanggalrincian->id}}/Edit">
                    @method('patch')
                    @csrf
                    <hr>
                    <div class="form-group row d-flex align-items-center">
                        <label for="" class="col-sm-4 col-form-label">Tgl Telah Menerima</label>
                        <div class="col-sm-1 text-right">:</div>
                        <div class=" d-flex col-7 justify-content-center">
                            <input type="text" class="form-control justify-content" id="" placeholder="" required name="Tgltelahmenerima" value="{{ $tanggalrincian->tgltelahmenerima }}">
                        </div>
                    </div>
                    <hr>
                    @if ($tanggalrincian != null)
                    <a class="btn btn-primary btn-md float-left" href="{{ Route('Tanggalrincian') }}/{{ $sppd->id }}">Selesai</a>
                    @else
                    @endif
