                ...
                <form method="POST" action="{{ Route('EntrySPPD') }}">
                    ...
                    <div class="form-group row d-flex align-items-center">
                        ...
                        <div class="col-sm-7">
                            ...
                            <select id="PUser" name="PilihUser" class="form-control" required autofocus>
                            <!--    <?php $no = 0; ?>
                                <hr>
                                @foreach ($usera as $user)
                                <option @if ($user == "Admin")
                                    selected
                                    @endif
                                @if ($user == "Staff")
                                selected
                                @endif
                                @if ($user == "Kepala Seksi")
                                selected
                                @endif 
                                value="{{ $no }}">{{ $user }}
                                <?php $no++; ?>
                            ...
                            @else
                            <input type="text" class="form-control justify-content" placeholder="Nama yang diperintah" required readonly value="{{ $user->nama }}">
                            <input type="text" class="form-control justify-content" placeholder="Nama yang diperintah" required readonly value="{{ $user->id }}" name="PilihUser" hidden>
                            @endif
                        </div>
                    </div>
                    <div class="form-group row d-flex align-items-center">
