...
class SPPDController extends Controller
{
    ...
    public function entry()
    {
        ...
        return view('sppd.users.entry', compact('user', 'tempatb', 'kabid', 'usera'));
    }
    public function storeentry(Request $request)
    {
        ...
        if ($validation) {
            ...
            User::find($request->PilihUser)->sppd()->attach($sppdid->id);
            Tempat::create([
                'sppd_id'           => $sppdid->id,
    ...
    public function showsppd($id)
    {
        ...
        $rincian = rincian::getRincian($id)->first();
        $laporan = laporan::getLaporan($id)->first();
        $tglsuratkwitansi = tgl_surat_kwitansi::select('tanggal_surat_kwitansi')->where('sppd_id',$id)->first();
        $tanggalkwitansi = tanggalkwitansi::getTanggalkwitansi($id)->first();
        $tanggalrincian = tanggalrincian::getTanggalrincian($id)->first();
        if ($this->CekUserSppd($user, $sppd)) {
            $sppd = $sppd->first();
            $sppd->update([
                'tgl_surat' => now()
            ]);
            return view('sppd.users.sppdoption', compact('user', 'sppd', 'rincian', 'laporan', 'tglsuratkwitansi', 'tanggalkwitansi', 'tanggalrincian'));
        } else {
            return view('sppd.users.akses', compact('user'));
        }
    }
    public function showsppdentry(request $id)
    {
        $user = User::getUser();
        $sppd = Sppd::getSppd($id);
        $rincian = rincian::select('sppd_id')->where('sppd_id',$id->id)->first();
        //dd($rincian);
        return view('sppd.users.sppdoption', compact('user', 'sppd', 'rincian'));
    }
